...
test('createTeam with valid name (happy path)', async () => {
  ...
  const result = await createTeamMutation.resolve(null, { input: { name: teamName } }, context);
...
test('createTeam fails with duplicate name (sad path)', async () => {
  ...
  await expect(
    createTeamMutation.resolve(null, { input: { name: teamName } }, context)
...
test('changeUserTeam assigns user to team (happy path)', async () => {
  ...
  const team = await createTeamMutation.resolve(null, { input: { name: teamName } }, context);
...
test('deleteTeam removes team (happy path)', async () => {
  ...
  const team = await createTeamMutation.resolve(null, { input: { name: `delete-team-${Date.now()}` } }, context);
